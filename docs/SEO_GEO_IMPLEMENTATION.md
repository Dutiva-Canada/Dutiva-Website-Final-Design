...
- **Search discovery is welcome** — general crawlers plus the AI _search_
  ...
  the `*` group entirely.
- **Foundation-model _training_ crawlers are not opted in** (`GPTBot`,
  `ClaudeBot` → `Disallow: /`). Training is a separate decision from search
  discovery; to change the policy deliberately, edit the `trainingBots`
  list in `scripts/prerender.mjs` and this document.
...
`validate-seo.mjs` parses every block and rejects off-origin URLs and
placeholder values.
