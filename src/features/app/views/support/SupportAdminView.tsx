...
export function SupportAdminView() {
  ...
  const [filters, setFilters] = useState<AdminTicketFilters>({ status: 'all', priority: 'all', category: 'all' })
    ...
    <div className="mx-auto max-w-[1180px] px-[28px] pt-[8px] pb-[64px] max-[640px]:px-[16px]">
      <header className="mb-[18px] flex flex-wrap items-baseline justify-between gap-[10px]">
        ...
        {tickets && (
          <span className="text-[13px] text-text-muted">
            {x(M.support_admin_open_queues)}: <span className="font-semibold text-text-2">{openCount}</span>
          </span>
        )}
      </header>
      ...
      <div className="mb-[16px] flex flex-wrap items-center gap-[10px]">
        <select
          ...
          onChange={(e) => setFilters((f) => ({ ...f, status: e.target.value as SupportStatus | 'all' }))}
          className={selectClass}
        >
          <option value="all">{x(M.support_admin_filter_status)}: {x(M.support_admin_filter_all)}</option>
          {STATUS_ORDER.map((s) => (
        ...
        <select
          ...
          onChange={(e) => setFilters((f) => ({ ...f, priority: e.target.value as SupportPriority | 'all' }))}
          className={selectClass}
        >
          <option value="all">{x(M.support_admin_filter_priority)}: {x(M.support_admin_filter_all)}</option>
          {PRIORITIES.map((p) => (
        ...
        <select
          ...
          onChange={(e) => setFilters((f) => ({ ...f, category: e.target.value as SupportCategory | 'all' }))}
          className={selectClass}
        >
          <option value="all">{x(M.support_admin_filter_category)}: {x(M.support_admin_filter_all)}</option>
      ...
      {error && (
        <p role="alert" className="m-0 rounded-[12px] border border-risk-border bg-risk-bg px-[16px] py-[12px] text-[14px] text-risk-fg">
          ...
          <table className="w-full border-collapse text-[13px]">
            <thead>
              <tr className="border-b border-border text-left text-[11.5px] tracking-[0.04em] text-text-muted uppercase">
                ...
                <th className="py-[8px] pr-[12px] font-semibold">{x(M.support_admin_col_requester)}</th>
                <th className="py-[8px] pr-[12px] font-semibold">{x(M.support_admin_filter_priority)}</th>
                <th className="py-[8px] pr-[12px] font-semibold">{x(M.support_admin_filter_status)}</th>
            ...
            <tbody>
              {tickets.map((t) => (
                <tr key={t.id} className="border-b border-inset hover:bg-inset">
                  ...
                  <td className="py-[10px] pr-[12px] text-text-2">{x(PRIORITY_LABELS[t.priority])}</td>
