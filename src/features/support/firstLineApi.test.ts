...
describe('getFirstLineAnswer', () => {
  ...
  it('sends the question with grounded Help Centre context and returns the answer', async () => {
    invoke.mockResolvedValue({ data: { data: { escalate: false, answer: 'Open Document Studio.' } }, error: null })
    const res = await getFirstLineAnswer('how do I make a document', 'product_question', [article], 'en')
    ...
    expect(opts.body).toMatchObject({ question: 'how do I make a document', category: 'product_question', language: 'en' })
  ...
  it('surfaces the server escalate flag', async () => {
    ...
    expect(await getFirstLineAnswer('x', 'privacy', [article], 'en')).toEqual({ escalate: true, answer: '' })
  })

  it('maps HTTP 429 to a rate_limited error', async () => {
    invoke.mockResolvedValue({ data: null, error: { context: { status: 429 } } })
    await expect(getFirstLineAnswer('x', 'product_question', [article], 'en')).rejects.toMatchObject({
      code: 'rate_limited',
    })
  })
