...
/**
 * Section bodies for `/guides`, keyed by the article's English slug.
 *
 * Split from the metadata in guideArticles.ts so the prose is not in the eager
 * entry graph: `src/seo/routes.ts` imports every article to build
 * `allPublicPages()`, and the router imports that — see
 * docs/TODO.md EF6b and scripts/check-entry-graph.mjs. Only ArticlePage reads
 * this module, and ArticlePage is a lazy route, so the bodies ride its chunk.
 *
 * Every key here must have a metadata entry and vice versa; `articles.test.ts`
 * fails the build if they diverge. An article with metadata and no sections is
 * a page that renders its title over nothing.
 */
